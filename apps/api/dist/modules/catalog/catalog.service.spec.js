...
describe('CatalogService', () => {
    ...
    beforeEach(async () => {
        ...
        const module = await testing_1.Test.createTestingModule({
            providers: [
                ...
                {
                    provide: (0, typeorm_1.getRepositoryToken)(channel_entity_1.Channel),
                    useValue: mockRepo,
                },
                {
                    provide: (0, typeorm_1.getRepositoryToken)(live_stream_entity_1.LiveStream),
                    useValue: mockRepo,
                },
                {
        ...
        channelRepository = module.get((0, typeorm_1.getRepositoryToken)(channel_entity_1.Channel));
    ...
    describe('getChannelsByCountryTopic', () => {
        ...
        it('should query database when cache miss and cache results', async () => {
            ...
            const queryBuilder = channelRepository.createQueryBuilder();
            ...
            expect(cacheManager.set).toHaveBeenCalledWith('channels:US:news:1:20', expect.objectContaining({ data: [mockChannel] }), 300000);
        });
    ...
    describe('getChannelById', () => {
        ...
        it('should throw NotFoundException when channel not found', async () => {
            ...
            await expect(service.getChannelById('nonexistent')).rejects.toThrow(common_1.NotFoundException);
        });
    ...
    describe('matchUrlToChannel', () => {
        ...
        it('should throw NotFoundException when no match found', async () => {
            channelRepository.findOne.mockResolvedValue(null);
            await expect(service.matchUrlToChannel('https://unsupported.com/video')).rejects.toThrow(common_1.NotFoundException);
        });
    });
    describe('searchChannels', () => {
        it('should perform full-text search with PostgreSQL', async () => {
            ...
            expect(queryBuilder.andWhere).toHaveBeenCalledWith('channel.search_vector @@ plainto_tsquery(:query)', { query: 'news' });
            expect(queryBuilder.orderBy).toHaveBeenCalledWith('ts_rank(channel.search_vector, plainto_tsquery(:query))', 'DESC');
        ...
        it('should handle empty search query', async () => {
            ...
            expect(queryBuilder.andWhere).not.toHaveBeenCalledWith(expect.stringContaining('search_vector'), expect.anything());
        });
    });
});
